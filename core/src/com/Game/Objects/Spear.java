...
public class Spear extends Weapon{
    public Spear(int x, int y, int damage, Player player, myGdxGame game) {
    ...
    public void attack(double time, int cd) {
        if (time % cd > 1 && time % cd < 1.5) {
            if (player.getLeft()) {
                ...
                animator.render((int) (player.getCenterX() + sprite.getRegionWidth() / 12), (int) (player.getCenterY() - sprite.getHeight() / 2));
            }
